...
export async function runAgentOnce(agentId: string): Promise<ActionRecord> {
  const { getAllAgents: _, ...db } = await import('../services/db.js');
  const agent = (await import('../services/db.js')).getAgentById(agentId);
  ...
  // Fetch balances
  const solBalance = await getSolBalance(connection, agent.publicKey);
  const splMintInfo = getActiveSplMint();
  const splBalance = splMintInfo
    ? await getSplBalance(connection, agent.publicKey, splMintInfo.mint)
    : 0;

  // Get cycle count for this agent (used in round-robin decision)
...
export async function runAllAgentsCycle(): Promise<ActionRecord[]> {
  ...
  for (const agent of agents) {
    try {
      ...
      results.push(result);
      await sleep(500); // Small delay between agents to avoid RPC hammering
    } catch (err) {
      logger.error({ agentId: agent.id, err }, 'Agent cycle error');
    }
  }
  return results;
}
...
interface ActionResult {
  status: 'SUCCESS' | 'FAILED' | 'SKIPPED_NO_ROUTE';
  signature?: string;
  explorerUrl?: string;
  amount?: number;
  error?: string;
...
async function executeAction(
  ...
  switch (type) {
    ...
    case 'ACTION_SPL_TRANSFER': {
      ...
      const amount = BigInt(1); // 1 base unit
